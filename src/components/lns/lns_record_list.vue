  ...
  <q-list link no-border :dark="theme == 'dark'" class="lns-record-list">
    ...
    >
      ...
      <q-item-section v-if="!isLocked(record)" side>
        ...
        <span v-else class="italonet-expiration">{{
  ...
  methods: {
    ...
    copyNameI18nLabel(record) {
      if (record.type === "session") {
        ...
        return "menuItems.copyItalonetName";
      }
    },
    copyValueI18nLabel(record) {
      if (record.type === "session") {
        return "menuItems.copySessionId";
      } else if (record.type === "italonet") {
        return "menuItems.copyItalonetAddress";
      }
      return "menuItems.copyAddress";
    },
    validMenuItems(record) {
      // change name depending on if italonet or session
      const lockedItems = [
    ...
    copyValue(record) {
      let message = this.$t("notification.positive.italonetAddressCopied");
      if (record.type === "session") {
